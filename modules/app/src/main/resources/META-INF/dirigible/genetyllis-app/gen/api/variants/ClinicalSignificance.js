...
const dao = require("genetyllis-app/gen/dao/variants/ClinicalSignificance");
...
rs.service()
	...
	.resource("count/{VariantId}")
		.get(function(ctx, request) {
			let VariantId = parseInt(ctx.pathParameters.VariantId);
			VariantId = isNaN(VariantId) ? ctx.pathParameters.VariantId : VariantId;
			http.sendResponseOk("" + dao.count(VariantId));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("ClinicalSignificance not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/v4/js/genetyllis-app/gen/api/ClinicalSignificance.js/" + entity.Id);
