...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'genetyllis-app.users.Notification.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('genetyllis-app.users.Notification.refresh', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	var api = '/services/v4/js/genetyllis-app/gen/api/users/Notification.js';
	...
	$scope.update = function() {
		if ($scope.entityForm.$valid) {
			$http.put(api + '/' + $scope.entity.NotificationId, JSON.stringify($scope.entity))
	...
	$scope.delete = function() {
		$http.delete(api + '/' + $scope.entity.NotificationId)
		.then(function(data) {
			$scope.loadPage($scope.dataPage);
			toggleEntityModal();
			$messageHub.messageEntityModified();
