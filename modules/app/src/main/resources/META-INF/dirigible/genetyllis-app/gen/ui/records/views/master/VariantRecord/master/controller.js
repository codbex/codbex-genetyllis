...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'genetyllis-app.records.VariantRecord.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('genetyllis-app.records.VariantRecord.refresh', callback);
		},
		onPatientModified: function(callback) {
			on('genetyllis-app.records.Patient.modified', callback);
		},
		onVariantModified: function(callback) {
			on('genetyllis-app.records.Variant.modified', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	var api = '/services/v4/js/genetyllis-app/gen/api/records/VariantRecord.js';
	...
	$scope.patientidOptionValue = function(optionKey) {
		for (var i = 0 ; i < $scope.patientidOptions.length; i ++) {
			if ($scope.patientidOptions[i].Id === optionKey) {
				return $scope.patientidOptions[i].Info;
			}
		}
		return null;
	};
