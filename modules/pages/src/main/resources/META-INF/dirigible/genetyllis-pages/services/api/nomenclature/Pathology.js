...
const dao = require("genetyllis-pages/services/dao/nomenclature/Pathology");
...
rs.service()
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("Pathology not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("filterPathology/{cui}")
		.get(function(ctx) {
			let cui = ctx.pathParameters.cui;
			let entities = dao.filterPathology(cui);
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/v4/js/genetyllis-app/gen/api/Pathology.js/" + entity.Id);
